...
async def update_user_score(user_id, score):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        # Вставляем новую запись или заменяем ее, если с данным user_id уже существует
        await db.execute('INSERT OR REPLACE INTO quiz_state (user_id, user_score) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET user_score = excluded.user_score', (user_id, score))
        # Сохраняем изменения
        await db.commit()
